...
**Architecture:** CI 用 `msys2/setup-msys2` 装 MSYS2 → robocopy 复制进 `workspace/tob/msys64` → 删包缓存瘦身 → 现有 ISCC 递归 glob 自动打包到 `{app}\msys64`。VBS 启动前把 `msys64\usr\bin` 与 `msys64\ucrt64\bin` prepend 到 PATH 并设 `CHERE_INVOKING=1`；`bash.py` 与 `.iss` 均不改。
...
**Files:**
- Modify: `workspace/tob/haitun agent.vbs`（在 .env 块之后、`objShell.Run` 之前插入）
...
Run: `grep -cE "^\s*(If |Do Until|End If|Loop)" "workspace/tob/haitun agent.vbs"`
...
- ISCC 步骤无需改动：现有 `Source: "*"; recursesubdirs createallsubdirs` 会自动把 `workspace/tob/msys64` 打包到 `{app}\msys64`。
...
**Files:**
- Modify: `workspace/tob/.gitignore`
...
Run: `grep -n "msys64/" workspace/tob/.gitignore`
...
**Files:**
- Modify: `workspace/tob/README.md`（Windows 安装包段，补 MSYS2 说明）
- Modify: `workspace/tob/AGENTS.md`（tools 表里 bash 一行）
...
把 `workspace/tob/AGENTS.md` 中这一行：
...
Run: `grep -n "msys64" workspace/tob/README.md workspace/tob/AGENTS.md`
Expected: README 与 AGENTS.md 各至少出现一处 `msys64`。

- [ ] **Step 5: 提交**

```bash
git add workspace/tob/README.md workspace/tob/AGENTS.md
...
- [ ] `pyinstaller.yml` 的 `haitun-inno-setup` job 内：`setup-msys2` 装包 → robocopy 进 `workspace/tob/msys64` → 删 pacman 缓存；YAML 可解析
- [ ] `.gitignore` 忽略 `msys64/`
