...
**Architecture:** A committed VBS launcher and Inno Setup `.iss` script live inside `workspace/tob/`. A new CI job (`needs: pyinstaller`, `runs-on: windows-latest`) downloads the Windows exe artifact, drops it into the workspace, installs Inno Setup via chocolatey, compiles the `.iss`, and uploads `Haitun Agent Setup.exe`.
...
**Files:**
- Move: `haitun.ico` → `workspace/tob/haitun.ico`

- [ ] **Step 1: Move the file with git**

```bash
git mv haitun.ico workspace/tob/haitun.ico
```

- [ ] **Step 2: Verify new location and old removed**

Run: `ls -la workspace/tob/haitun.ico && ls haitun.ico 2>&1 || echo "root copy gone (expected)"`
...
**Files:**
- Create: `workspace/tob/haitun agent.vbs`

- [ ] **Step 1: Write the file**

Create `workspace/tob/haitun agent.vbs` with EXACTLY this content:
...
Run: `cat "workspace/tob/haitun agent.vbs"`
Expected: the 4 lines above, exactly.

- [ ] **Step 3: Commit**

```bash
git add "workspace/tob/haitun agent.vbs"
...
**Files:**
- Create: `workspace/tob/haitun.iss`

- [ ] **Step 1: Write the file**

Create `workspace/tob/haitun.iss` with EXACTLY this content:
...
Notes:
- `Source: "*"` + `recursesubdirs createallsubdirs` packages the whole workspace tree (systems/, tools/, skills/, …) plus `psi-agent.exe`. Source paths are relative to the `.iss` directory (`workspace/tob/`).
...
Run: `cat workspace/tob/haitun.iss`
Expected: the content above, exactly.

- [ ] **Step 3: Commit**

```bash
git add workspace/tob/haitun.iss
...
**Files:**
- Modify: `workspace/tob/.gitignore` (lines 29, 31 — remove `haitun.ico` and `haitun agent.vbs`)
...
Delete the line containing exactly `haitun.ico` from `workspace/tob/.gitignore`.

- [ ] **Step 2: Remove the `haitun agent.vbs` line**

Delete the line containing exactly `haitun agent.vbs` from `workspace/tob/.gitignore`.

- [ ] **Step 3: Verify**

Run: `grep -nE "haitun.ico|haitun agent.vbs|psi-agent.exe" workspace/tob/.gitignore`
Expected: only `psi-agent.exe` remains; the other two are gone.

- [ ] **Step 4: Commit**

```bash
git add workspace/tob/.gitignore
...
**Files:**
- Modify: `workspace/tob/README.md` (append a short "Windows 安装包" section)
...
Run: `tail -12 workspace/tob/README.md`
Expected: the new section is present.

- [ ] **Step 4: Commit**

```bash
git add workspace/tob/README.md
...
- [ ] `workspace/tob/` contains `haitun.ico`, `haitun agent.vbs`, `haitun.iss`
