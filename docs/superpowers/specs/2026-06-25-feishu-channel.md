...
| 修改 | `workspace/tob/TOOLS.md` | 常驻引导：群聊里如何用 `feishu_message_list` / `feishu_doc_read` / `feishu_file_download` |
...
| 修改 | `workspace/tob/tools/_feishu_impl.py` | `subscribe_approval_impl` / `unsubscribe_approval_impl` + 两个 `_build_*_request` |
| 修改 | `workspace/tob/tools/feishu_approval.py` | `feishu_approval_subscribe` / `feishu_approval_unsubscribe` 薄封装 |
| 修改 | `workspace/tob/TOOLS.md` | 订阅审批事件的运行时引导（订阅一次、不轮询、收 `<feishu_approval_event>`） |
...
| 修改 | `workspace/tob/tests/test_feishu.py` | subscribe/unsubscribe 建请求 / 校验 code / 透传错误 / 工具 async 带 docstring |
