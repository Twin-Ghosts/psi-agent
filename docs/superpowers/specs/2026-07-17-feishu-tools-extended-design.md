...
与基础文档工具，它们各有独立规格）。全部落在 `workspace/tob/` 内，`src/`
零改动、零新增依赖。
...
| 修改 | `workspace/tob/AGENTS.md` | 工具表补 `feishu_sheet` 行（此前连写工具都没登记）；技能列表补 `feishu-todo-board-sync` |
...
| 新增 | `workspace/tob/tools/_oauth_receiver.py` | `plan_receiver`（通道择优）/ `wait_loopback`（一次性回环监听，校验 state）/ `poll_gateway`（轮询取件） |
...
| 修改 | `workspace/tob/AGENTS.md` | 工具表补 `feishu_auth` 行；环境变量表补两个 `PSI_OAUTH_*` |
...
| 新增 | `workspace/tob/tests/test_oauth_receiver.py` | 通道择优 6 例 + 真实回环往返（成功 / state 不匹配后仍等真回调 / 错误回调 / 超时 / 无基址不轮询）（11） |
| 修改 | `workspace/tob/tests/test_feishu.py` | authorize URL 带 PKCE 且 verifier 落盘、`auto_receive` 两路径、换 token 带 verifier/redirect_uri、`auth_wait` 5 例 |
