...
- [ ] 构建镜像用 **`fix/external-container-recovery-v2`**（HEAD `520a5924`，基于
      `a9579c7d`，已推远程）。**不要用 `fix/external-container-recovery`**，理由见阶段 1
- [ ] 照 `psi-agent-release-to-cloud.md` 执行，同一窗口内一并落地记忆服务 `.env` 改动
- [ ] 按 D4 决策处置两个容器的历史/记忆/文件（归档必做，迁移不在本阶段）
- [ ] 补成 xx 容器缺失的 `memory_tokens.json`
- [ ] 按 §5 八条判据 + 本文 V1-V10 验收。**V11 降为观察项**——出向发文件的链路阶段 1
      没有修（记录⑨），只记实测现象，不作通过判据
- [ ] 开工前须负责人显式确认（本阶段改生产环境；依赖链已解除，不会被自动带起来）
...
>
> **当前只有 V8 可判。** V1-V7、V9-V11 全部依赖阶段 2 的真实发布——它们判的是
> **生产环境的状态**，而阶段 1/3 一次生产写操作都没做。这不是没测，是**还没到能测的时候**。

| 编号 | 结果 | 证据 |
|---|---|---|
| V1 | **待验**（阶段 2） | 需发布后重新在容器内比对 99 个 `.py`。当前实测值是**发布前**的基线：相同 93 / 不同 6 / 缺失 0 |
| V2 | **待验**（阶段 2） | 需发消息看两侧历史行数 |
| V3 | **待验**（阶段 2） | 同 V2。注意成 xx 容器**从未工作过**，这条是首次验证而非回归验证 |
| V4 | **待验**（阶段 2） | 两个 `grep` 需在新容器内跑 |
| V5 | **待验**（阶段 2） | 依赖 `.env` 落地 `FUSION_MEMORY_AUTO_REGISTER_FEISHU` |
| V6 | **待验**（阶段 2） | 21 名存量用户需逐个对 token 表 |
| V7 | **待验**（阶段 2） | 归档前后对账 |
| **V8** | **通过** | 见下方「怎么测的」 |
| V9 | **待验**（阶段 2） | 照发布文档 §5 逐条执行 |
| V10 | **待验**（阶段 2） | 需发布后确认卷与表数 |
| V11（观察项） | **待观察**（阶段 2） | 预期仍是裸 `[SEND:]` 文本，阶段 1 未修出向 |
...
**其余 V1-V7、V9-V11 的测法在阶段 2 卡里已写死**，此处不重复。要点：V2/V3 是关键
（独立容器侧历史增、主容器侧不增，才算路由真修好），V10 确认 pgvector 未丢。
