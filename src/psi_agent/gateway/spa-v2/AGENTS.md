...
- SSE `reasoning`：**刻意压缩**仍走同一字段；用 `kind`（`thinking` / `tool_call` / `tool_result`）区分——**≠** `/history` 消息 provenance `kind`。过程轴见 `services/turnProgress.ts`（对标 Cursor）：
  ...
  - **回合结束后「已思考」可展开（对标 Cursor）**：流式期间仍只显示过程轴；回合成功/失败结束且 agent 气泡仍在时，把本轮累积的 SSE `reasoning` 挂到 `message.reasoning`。`FocusChatThread` 在正文上方显示可折叠「已思考」（默认收起）；正文用 `stripToolMarkersFromReasoning`（`services/reasoningDisplay.ts`）只展示思考散文，工具标记不进展开区。`/history` 透出 JSONL `reasoning`；无正文的 tool 轮思考由 Gateway **折叠**进下一条可展示 assistant；`historyToChat` 合并连续 assistant 时一并拼接。刷新同任务即可展开，不必新建任务。无思考散文（仅 tool markers）则不渲染该入口。
