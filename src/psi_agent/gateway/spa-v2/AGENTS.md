...
- SSE `reasoning`：**刻意压缩**仍走同一字段；用 `kind`（`thinking` / `tool_call` / `tool_result`）区分——**≠** `/history` 消息 provenance `kind`。过程轴见 `services/turnProgress.ts`（对标 Cursor）：
  ...
  - **回合结束后过程拆分封装（对标 Cursor）**：流式期间仍只显示过程轴；回合结束把累积 SSE `reasoning` 挂到 `message.reasoning`。`FocusChatThread` 用 `parseReasoningSegments` **分开两块**（工具优先）：①「已调用 N 个工具」——短句列表（与过程轴封存行同款 `summarizeToolCall`，**默认展开**）；②「已思考」——仅思考散文（含工具轮之间的中间想法），**默认收起**。`tool_result` 不单独成行。仅有工具、无散文时也显示工具块。`/history` 透出 JSONL `reasoning`，并把各轮结构化 ``tool_calls`` **合成** `[Tool Call:…]` 再折叠进可展示 assistant（Session **刻意**不把该标记写入 JSONL，只 SSE 流式发出）。`historyToChat` 合并时拼接。刷新同任务即可还原工具列表 + 思考。
