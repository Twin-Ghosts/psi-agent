...
export function summarizeToolCallText(text: string): string | null {
  const match = text.match(TOOL_CALL_FULL) ?? text.match(/\[Tool Call:\s*([A-Za-z0-9_.-]+)/);
  if (!match) return null;
  return summarizeToolCall(match[1] ?? "", match[2] ?? "{}");
}

export function pushSummary(log: ProgressLog, summary: string): ProgressLog {
  const last = log.lines[log.lines.length - 1];
  if (last === summary) {
    return { lines: log.lines, current: TURN_PROGRESS.planning };
  }
  return { lines: [...log.lines, summary], current: TURN_PROGRESS.planning };
}
