...
| **`get_session_id()`** | 仅 **workspace 工具**需要「当前会话 id」时（如 `todo`、fusion memory，以及 `_runtime_paths` 的飞书隔离守卫——它从 session_id 反解 owner 判权）。框架内部用 `Conversation.session_id` / 显式参数 |
