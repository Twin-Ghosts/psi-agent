...
└── feishu/
    ...
    └── client.py       # Bot handler + 卡片流式 + 文件收发 + 处理状态表情 + 按用户独立会话路由 + 审批事件推送 (~760行)
```
...
- **按用户独立会话与附件渠道（`gateway_url`，默认 None）**：设置后同一飞书机器人对不同飞书用户提供**各自独立**的 Session；多个 Session 仍可共用同一个 `examples/haitun-workspace` Agent 能力包，隔离的是 Session 用户数据 workspace 与历史。`run_feishu` 用 `AsyncExitStack` 持有所有 per-user `ChannelCore` + 一个 REST `aiohttp.ClientSession`；路由回调**在白名单通过后才解析**（被拦用户不建连接），按 `open_id` 经 `_GatewayRouteProvider.ensure` → Gateway `POST /feishu/route` 幂等拿回 `{open_id, session_id, channel_socket, workspace}` 完整路由并缓存，再经 `_CoreRegistry.get` 缓存复用对应 `ChannelCore`。**路由/spawn/workspace 决策权全在 Gateway 侧的 `FeishuManager` / `SessionManager`**，channel 只消费实际 route，不自行拼 `<root>/<open_id>`、不 spawn、退出也不删。消息必须先路由成功再 `_build_chunks`；附件写到 `<workspace>/uploads/<date>/<safe-message-id>/`，文件名/资源 key 做路径 component 编码。Agent 返回的 `FileChunk` 在发送前对文件与 workspace 做异步 `resolve(strict=True)` containment 检查，因此 traversal、跨用户 workspace 和 symlink 逃逸均拒绝且不向用户暴露宿主路径。该边界只约束 Feishu 附件落盘/回传，**不是 OS/container 沙箱**，不修改通用 workspace 工具的绝对路径能力。**（刻意为之，fail-closed）** 配置 Gateway 后，缺 open_id、Gateway 不可达、非 201 或响应缺字段都拒绝消息，绝不回退共享 `session_socket`；失败不写 route 缓存，下条消息重试。`gateway_url=None` 时行为与原来一致（全体共用 `session_socket`，附件仍落用户 Downloads）。**（刻意为之，取消安全）** `AsyncExitStack` 与 `BlockingPortal` 的进出顺序保持 portal 先关，保证在飞 handler 仍可使用 core/http；两个 registry 继续用 dict 快路径 + `anyio.Lock` double-checked 消除并发重复创建。
