...
5. **System prompt 容错**：`system_prompt_builder()` 可能抛异常或返回 None。首次 `run()` 调用时必须 catch 异常，不影响后续对话（此时 history 中没有 system 消息）。同理 `system_prompt_dynamic_suffix()`（每回合刷新易变尾部）失败、返回非 `str` 或空串时一律**保留原提示词**——时钟走慢远好过 system prompt 被截断或抹空（见坑 19）
...
19. **system prompt 只刷易变尾部、不整段重建（刻意为之，勿"优化"成每回合整段重建、也勿删掉那几个 no-op 分支）**：`SystemPrompt.ensure()` 每回合都跑，三条路径按优先级二选一——history 空则整段构建；`system_prompt_rebuild_checker()` 返回 True 则整段重建（**不再叠加刷新**，产物已最新）；否则调 agent 包的 `system_prompt_dynamic_suffix(current)` **只重渲染缓存边界之后的尾部**。缺了第三条，提示词就是「首个回合建一次、整段沿用到会话结束」，里面**所有描述「现在」的内容全部冻结**：7月24日建的会话连着几天说今天是 7月24日；构建那刻算错的时区标签（容器 `TZ` 未生效 → `Asia/Shanghai` 记成 `UTC`）活到会话结束；agent 照读陈旧时间作答、被追问时还会编一套时区换算圆场（真实事故）。同样被冻的还有 haitun 设计在边界之下的 `USER.md` / `HEARTBEAT.md`（文档写明「re-read every turn」，实际改了永远不生效）。**为什么不整段重建**：一是整段构建要重扫 skills/tools/bootstrap，实测 haitun 约 110ms、150KB 提示词；二是会改动**被缓存的前缀**、每回合击穿上游 prompt caching——workspace 把提示词切成 `stable_prefix + CACHE_BOUNDARY + dynamic_suffix` 本就是为避免这件事。只刷尾部则稳定前缀**逐字节复用**，实测每回合约 4.6ms（约 1/22）。**边界常量属于 workspace，内核不认识它**：内核只把旧串交出去、拿新串写回，切在哪由 workspace 定。五个 no-op 分支都是刻意的容错，**别当冗余删掉**——未定义 refresher（老 workspace 行为不变）、首条消息不是 `system`（压缩后的历史可能不以提示词开头，`messages[0]` 根本不是它）、找不到边界（提示词可能来自别的 builder 或已被压缩改写，替换定位不到的尾部会毁掉整段）、刷新抛异常、返回非 `str` 或空串（会把 system prompt 抹空）：一律保留原提示词，因为**时钟走慢远好过 system prompt 被截断**。`refresher` 保持 `None` 默认而不套用坑 8 的「Default over None」：默认函数只能返回「原样」，与 `None` 语义重合却多一次无谓 await，且 `None` 本身承载「这个 workspace 没有分层提示词」的语义。详见 `session/AGENTS.md`「System prompt 生命周期」。
