...
└── psi_agent/
    ...
    └── gateway/                     # 骨架层：两条产品线的共同装配 + 公共端点
        ...
        └── feishu/                  # **ToB 专属层**
            ├── _routes.py          # register_feishu_routes + /feishu/route /feishu/routes /feishu-web/；register_oauth_routes + /oauth/callback /oauth/code
            ├── _feishu_manager.py  # FeishuManager — 飞书 open_id → Session 路由
            ├── _oauth_manager.py   # OAuthRelay — OAuth 回调中继（免手抄授权码；取件方全在 ToB 一侧）
            └── _openapi.py         # ToB path 片段（FEISHU_PATHS /feishu/*）+ OAUTH_PATHS（/oauth/*，与产品线正交）
```
...
- **一棵树只改一个区**：前端树只碰 `src/psi_agent/gateway/desktop/spa-v2/`（及必要的 Gateway 壳 / spa v1）；workspace 树主要碰 `workspace/tob/`，以及必要的 Session / Gateway 服务端。越区改动优先换树，而不是在本树顺手改
...
- **阅读顺序**：根 `AGENTS.md` → `session/AGENTS.md` → `gateway/AGENTS.md` → `workspace/tob/AGENTS.md` 或 `gateway/desktop/spa-v2/AGENTS.md`
