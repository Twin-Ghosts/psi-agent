...
└── psi_agent/
    ...
    ├── _private_space.py       # 飞书按人对称隔离守卫（gateway↔channel↔workspace 工具共享）
...
22. **飞书按人对称隔离：守卫默认关闭、读松写紧、无主 session 不受约束（三条都刻意为之，勿"修掉"）**：`_private_space.py` 让每个飞书会话只能读写 `<root>/<owner>/`，owner 由 `owner_from_session_id()` 从 session_id 反解（私聊 `<open_id>`、群聊 `chat-<chat_id>`）。三个反直觉点：**(a) 默认关闭**——`PSI_WORKSPACE_ROOT` 未配时 `enabled()` 为 False，所有守卫函数退化成空操作、行为与改动前逐字节一致（故能先部署再开关，由 Gateway 按 `--feishu-workspace-root` 设置），别把那些"看着永不生效"的分支当死代码删掉；**(b) 读松写紧**——读放行「自己的空间 + 公共区 `public/` + root 之外（agent 包 / 系统路径）」，只拦别人名下的；写则连公共区也拒，否则一个人能覆盖公共材料影响所有人；**(c) 无主 session 完全不受约束**——SPA 手建的 session 没有 `feishu-` 前缀，`owner_from_session_id()` 返回 `""`，直接放行，因为那是本机用户自己的会话。**唯一耦合点**：`FeishuManager._workspace_for()` 复用 `owner_from_session_id(_session_id(key))` 推导目录名，守卫判权靠「反解出的 owner == 路径首段」；两边命名规则一旦漂移，后果**不是放行而是所有人都读不到自己的文件**（静默失效），已由 `test_workspace_dirname_matches_guard_owner` 钉死。**能力边界（如实记录）**：路径工具经 `resolve_under()` 一处收口是确定性判定；`bash`/`powershell` 只能对命令串做**字符串启发式**扫描，挡得住顺手 `cat` 别人目录，挡不住变量拼接 / base64 / 中转文件——要强隔离得每人独立容器。这道缺口靠加强字符串扫描补不上（等于跟图灵完备的 shell 打对抗），故另在系统提示词里加了 `PRIVATE_SPACE_SECTION`（`prompt_sections.py`，仅 `enabled()` 为真时拼进 `stable_parts`），明确要求 agent 拒绝协助绕过。**那是软约束，不是安全边界**：它约束的是模型自身行为，会随提示词改动 / 对话上下文 / 模型版本漂移，别因为「立了规」就把它当成前面那条能力边界已经补上——对外描述时两者必须分开讲。另注：`rg` 的 `--glob` 排除只是**加速**，因它按路径末段匹配而边界目录是绝对路径（实测会漏），准确性靠对输出按真实前缀再过滤一遍，那层别当冗余删掉；部署环境（214 容器）**没装 rg**，走的是纯 Python walk 分支，两条都要有边界。跨 session 历史工具同样要按主过滤——AppData 的 `histories/` 是**全局**的（不分 workspace），对话原文比生成的文件更敏感。**隔离只覆盖开启之后新建的文件**，存量文件仍在原处，报「已隔离」时必须一并说明。详见 `gateway/AGENTS.md`「FeishuManager」与 `channel/AGENTS.md`「`[SEND:]` 判权」。
