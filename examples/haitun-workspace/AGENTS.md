...
| `handbook_onboarding_configure` / `handbook_onboarding_show_config` / `handbook_onboarding_send_welcome` / `handbook_onboarding_process_submit` | 入职管理制度确认：NL 配 SOP 链接与 HR 通知（写 `config/handbook_onboarding.yaml`，勿让用户手改）→ 欢迎卡 → 校验 → 通过双侧通知 / 失败重发新卡。触发器 `handbook-onboarding-welcome` 挂 `feishu.hr.user_created`。skill：`feishu-handbook-onboarding`。 |
...
- `feishu-handbook-onboarding` — 入职管理制度确认闭环：NL 用 `handbook_onboarding_configure` 写入 SOP 链接/通知对象（勿手改 YAML）；`feishu.hr.user_created` → `handbook_onboarding_send_welcome`；卡片提交 → `handbook_onboarding_process_submit`。
