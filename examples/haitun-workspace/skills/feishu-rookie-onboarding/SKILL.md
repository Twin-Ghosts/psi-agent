...
3. 选「非研发」→ 其余 5 个 `dev_only` 项标 `不适用`（不计进度分母、不催办、不进 HR 日报），
   `role_confirmed` 本身保持已完成、不受影响。
4. 选「研发」→ 工具会发一张**新卡**，`role_confirmed` 显示为已完成（无按钮），下面展开 5 个
   研发项供勾选。这是刻意的：原卡按钮点完即被消费，`feishu_message_edit_card` 不重新注册
   回调，编辑出来的按钮全是死的。
