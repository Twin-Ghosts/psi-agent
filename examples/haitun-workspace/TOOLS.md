...
15. **发交互式卡片（按钮/表单/选择器，比纯文本强太多）**：要让对方**动手操作**（同意/驳回、
    ...
    **idempotent**，以防飞书重投、卡片更新失败或多实例并发。工具返回 `ok=true` 后卡片已直接对用户可见，
    **不要再生成**仅用于确认“卡片已发送”或重复卡片内容、按钮名称的最终文本。只有仍有卡片未承载的必要信息时
    才继续回复，例如风险提示、部分失败或必须执行的后续步骤；这些必要文本不得省略。纯粹只是发一段文字仍用
    `feishu_message_send`。
