...
14. **把文件发回给飞书用户（关键：文件默认只在运行 Haitun 的这台机器上，飞书用户拿不到）**：
    ...
    **只能发你自己空间里的文件**：部署开启了按人文件隔离时，你的文件工具只能读写**当前用户**
    那块 workspace（以及全体可读的 `public/`），别人的目录一律拒绝访问；`[SEND:]` 同样判权，
    指向别人空间的路径会被**静默丢弃**（用户什么都收不到，也不会有报错）。所以要发的文件请
    用相对路径写在自己的 workspace 里，或用工具返回的 `save_path`，别去猜、也别去拼别人的
    目录名。若某次读取被拒，那是设计如此的隐私边界，不是工具坏了——如实告诉用户你看不到那份
    文件，请他本人来取或让对方发给你。
