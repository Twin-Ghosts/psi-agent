...
async def feishu_bitable_create_app(name: str, folder_token: str = "", time_zone: str = "", user_key: str = "") -> str:
    ...
    Args:
        ...
        user_key: The sender's open_id (from ``<feishu_context>``). Pass it so the base
            is created under that user and owned by them; empty uses the bot's tenant
            token (the base then lives in the bot's space and the user needs to be given
            access).
    """
    return _f.dumps_result(await _f.create_bitable_app_impl(name, folder_token, time_zone, user_key))
...
async def feishu_bitable_create_table(
    ...
    return _f.dumps_result(
        await _f.create_bitable_table_impl(app_token, table_name, fields_json, default_view_name, user_key)
    )
...
async def feishu_bitable_create_field(
    ...
    return _f.dumps_result(
        await _f.create_bitable_field_impl(
            app_token, table_id, field_name, field_type, property_json, ui_type, user_key
        )
    )
