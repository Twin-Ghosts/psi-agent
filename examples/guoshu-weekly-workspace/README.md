...
└── tests/
    ├── smoke_test.py          # 317 条契约断言，不花模型 token
...
| `weekly_freshness_distribution` | 新鲜度分桶（30/90/180 天，**可加 `in_flight` 只看在办：从未报进展在办 8 条、不限状态 9 条**，随返回给 `task_total` 供各档自校）、自定义天窗、**时间漂移检出（73 条，双向不一致）**、滞后清单（含从未上报）、近期上报清单 |
...
| `weekly_milestone_stats` | 里程碑统计（完成率/多维分解/软删审计/**里程碑被全删的 3 条任务**/每任务分布（**带 `top_tie_count`：最多那档 23 条并列**）/任务与里程碑错配） |
...
| 评测 | 317 条契约断言 + 396 题基线 | 再加 200 题真实库集 + 多轮追问集 |
