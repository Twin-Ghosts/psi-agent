...
28. 不要按对象逐个调明细工具去凑一份清单。凡是「哪些任务/每个看板各多少/一个都没有的」
    ...
    工具说明里写了「要某类问法必须带某参数」（如「当期/前 N 条」带 order_by=progress_time）
    的，不带就是拿错误的一页当答案——按说明带参数再作答，不要用默认排序硬答。
...
76. 「负责人」在清单与统计两类问句里语义不同，先分清：
    ...
    - 问「任务量最大的负责人／牵头人」（统计排名）：才按角色分——「负责人／主责人」
      用 scope=workload role=owner（姚立诚 7 条），「牵头人／分管领导」用 role=lead_owner
      （吴晓东 14 条）。两个角色两套数字，不要拿牵头人的榜答负责人的问。
