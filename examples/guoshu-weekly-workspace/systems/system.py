...
66. 上一条的两个口径在「多列汇总」里不能自由选：问「各组／各看板的客观指标」这类
...
67. 相对时间窗的天数是口径，不是可以随手取的默认值。问句只说「滞后／很久没上报／
...
68. 同一份「新鲜度」有两根判据，别混用：task.latest_progress_time 是任务表上的
...
69. 「几条」和「占比」是两问，闸门跟着问句变，不要一套参数答两题。问「各组有几条任务
