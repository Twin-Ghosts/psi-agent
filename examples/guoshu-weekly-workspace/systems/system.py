...
- weekly_milestone_stats：里程碑完成率的各维度聚合（清单类问题才用上一个）。
  问「按承担组看里程碑数量和完成率」用 scope=by_dimension by=category：承担组是里程碑
  自己的类别列（区域组/安全组/技术组/标准组/运营组/总体组 6 档），不是任务的专项组
  （project_group 11 档）——拿 by=project_group 答会得出另一批组名。
...
6b. payload（草稿快照）的键名可查、键值不可读，这是两件事，别一起拒掉：
    ...
    payload_key_combos 按 board_code 分行（同一种键名组合在技术组与集团组各有一行），
    逐行报、不要跨看板合并数字。
    问「当时填的是什么内容」「成效字段填了什么」才是要键值，那个仍不可答，
    按权限说明即可：确认涉及哪些单（一次调用）后直接说「payload 键值按权限不展示」，
...
41. 名次题（前 N 条、报得最多的几条）并列一律按 task id 定序，不按任务名。11 期的有 8 条
    ...
    **「前 N 条」与「最……的那一条」的并列处理不同**：问句给了数量（前 5 条 / 前 8 条）时，
    一律按工具的返回顺序硬切 N 行——榜首并列也只取前 N 行，并列情况用一句话说明，
    不要把并列行都展开进答案集合（工具已按定序键排好，直接截取返回行，不要自己重排）；
    问句没给数量、问「谁最多／哪一条」时，才取首行并把并列在文字里说明。
...
75. 计数题的范围词决定闸门，先看问句再选口径：
    - 「一共／总共有多少条」没给限定词时 = 全表行数（年度目标 387、task_progress 全表 1068）；
      加了「正式任务／已发布／对外展示」才上对应闸门（对外展示 = 已发布 943）。
    - 「进展／报了进展／更新过进展」默认指已发布进展行（规则 69），草稿与未发布的不算
      （8 月那 4 条草稿不算「报了多少条进展」）。
    - 「这个月」= 快照所在月（2026-08），不是机器当月；进展按月的节奏由 progress_date 定。
76. 「负责人／主责人／谁手里」默认指任务主责人（task.project_owner_id 口径，scope=workload
    role=owner，姚立诚 7 条）；「牵头人／分管领导」才是 lead_owner（吴晓东 14 条）。两个角色
    两套数字，不要拿牵头人的榜答负责人的问，也不要反过来。
