...
- Per-user file isolation: once `--feishu-workspace-root` is set, files and conversation histories are **mutually invisible** across chats — each party may only read and write its own `<root>/<owner>/`, `<root>/public/` is world-readable but not writable, and a group shares `chat-<chat_id>`. **Separate directories are only a grouping; the isolation comes from boundary checks in the tool layer** (the shared exit of 22 path tools, shell command scanning, walking tools, tools doing their own IO, `[SEND:]`, and cross-session history tools). Two caveats: the shell layer is a string heuristic rather than a sandbox — it stops a stray `cat` of someone else's directory but not deliberate variable splicing or base64; and **isolation only covers files created after it is switched on**, so pre-existing files already sitting in a shared directory still need migrating. For strong isolation, give each user their own container
