...
- Per-user isolated sessions: with `--gateway-url http://127.0.0.1:8080`, on each Feishu user's first message the Gateway idempotently spawns a dedicated Session keyed by their open_id (isolated user-data workspace subdir and history), while all Sessions may keep using the same `examples/haitun-workspace` Agent package. Attachments are stored under the routed Session workspace at `uploads/<date>/<message_id>/`; with `--feishu-workspace-root users`, the complete layout is `users/<open_id>/uploads/...`. The mounted AI and workspace parent dir are set via the Gateway's `--feishu-ai-id` / `--feishu-workspace-root`. In Gateway mode, route failures reject the message instead of falling back to the shared socket. Without `--gateway-url`, all users share `--session-socket` and the legacy download directory (unchanged behavior)
