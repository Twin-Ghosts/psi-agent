...
| GET | `/oauth/callback` | OAuth 重定向落地点：收下 code 和 state 交给中继 |
| GET | `/oauth/code` | 发起方按 state 取件授权码并作废 |
...
| GET | `/workspace/places` | PathPicker 快捷位置 |
| GET | `/workspace/file` | 读取文件为 base64 |
| POST | `/workspace/reveal` | 在本机文件管理器中显示路径 |
| POST | `/ui/attention` | 触发注意提醒（闪烁托盘/webview） |
