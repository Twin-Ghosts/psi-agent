...
| `web_search` (`web_search.py` + `_web_search_impl.py`) | **默认网页搜索**: 国内**博查 Bocha**（`POST https://api.bocha.cn/v1/web-search`，控制台 open.bocha.cn）。纯 aiohttp（已是核心依赖），无需额外包、无需代理；要求 `BOCHA_API_KEY`。支持 `freshness` 时间范围 / `site` 站内 / `summary` AI 摘要 / `count` 最多 50。国内低延迟、中文收录好，日常搜索首选。 |
| `search` (`search.py` + `_mcp.py`) | **可选的 Serper（Google/英文/垂直搜索）补充**，仅当配置 `SERPER_API_KEY` 时可用（境内直连 Google 系通常不稳，且只覆盖 Google 收录）。**`serper_google_search` 常驻**（Google 网页搜索）；图片/地图/学术/专利/新闻/购物/评论等 12 个垂直搜索走 **`serper_call(tool, args_json)`**，参数表在 `serper-mcp` 技能里。常规搜索优先 `web_search`，需要 Google/英文/垂类结果时才用本组。 |
...
- **Web search (Bocha, 默认)**: 设置 `BOCHA_API_KEY`（博查开放平台 https://open.bocha.cn 申请, 新账号有免费试用包）。纯 aiohttp（已是核心依赖），无需额外包与代理。
- **Serper search (可选, Google/英文/垂直搜索补充)**: install psi-agent with the `mcp` extra, have `uvx` available, and set `SERPER_API_KEY`。
